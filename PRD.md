...
| D32 | **Menus are the full active word list with soft priority (2026-08-05, founder).** Every generation is offered all learning-status words except those at their D27 daily cap, ordered most-wanted-first (due words, then fewest lifetime servings); the prompt says to prefer earlier words when fits are equally natural but embed freely. The offered menu is logged per piece (`offered_words`), making offered-vs-embedded skip rates first-class data. **The proprietary channel (when built) takes the inverse priority: rarely-landed words get premises engineered for them** | Founder: density is the top worry — too much text, too few rewritten words. The 12-word menu cap (an implementation shortcut, never a decision) made menu composition a policy problem and starved density as easy words rotated out (day-5 evidence: menus of leftovers, 3-block pieces). Full choice raises the density ceiling; D27 caps stay the only hard constraint; QC guards quality; rewrites land words opportunistically, proprietary content targets the hard ones |
...
| D36 | **Engine mode = rewrite on gemini-3.1-flash-lite, with relaxed fidelity (2026-09-24, founder).** After G2 (5 real pieces: substitute ≈1 slot/piece, rewrite 8/1,000 words but via invented clauses), the founder ruled: the AI may add phrasing "once in a while" to embed a word; the reader is told to refer to the source for anything important. No stronger/costlier model (D5 stands; Anthropic runs not to be pushed). Assistant's definition line, accepted as the implementation: added *phrasing* is allowed, added *facts, figures, quotes or attributions* are not — `prompts/transform.md` says so, and the disclaimer on every piece now reads "Adapted with AI to carry your words: phrasing may be added or changed, facts should not be — check the original for anything that matters." | Density is the product (D33's lesson); substitute-only starves it on real reading. Measured caveat on record: in the G2 rewrite run the invention was per-embed, not occasional, and once fabricated an attribution on a news quote — hence the fact/attribution line. Fact-level QC remains a revisit trigger if TestFlight readers flag distortions |
| D37 | **Link intake is out of the MVP (2026-09-24).** Reddit and X posts reach RetAIn as user-selected text; web articles via the in-Safari extension; a server-side URL fetch only as a fallback for URL-only shares from non-Safari apps. Backlog M7 parked. | docs/link-intake-legality.md: Reddit's unauthenticated `.json` shut 2026-05-28 (403 verified) and its Developer Terms exclude monetised apps; X has no free tier and oEmbed truncates; both licences allow "format for display" only; Apple 5.2.2 binds third-party terms at review |

| D38 | **M1 transform service: architecture and hosting (2026-09-24, founder).** Proposal in docs/architecture.md accepted: Python + FastAPI + uvicorn service wrapping the existing engine; SQLite (users / words / pieces / events / calls); Sign in with Apple → own session token; server-sent events for the transform phases; per-user daily cap; keys server-side only. **Hosting: the Mac mini via Tailscale Funnel now; Fly.io (~$5/month, founder-approved) if the Mac becomes a problem, and before anyone outside TestFlight uses it.** Dependencies approved on the condition they are best-practice, open-source, well-supported and documented (FastAPI MIT, uvicorn BSD, PyJWT MIT). | Alternatives weighed: no server / BYO key (no metering, key on device, forecloses subscription); serverless functions (worst fit for 5–30 s streamed requests, two paid services on day one); BaaS (still needs a long-running engine process, lock-in). One process + SQLite covers ~1,000 daily actives; the scale path (two machines + Postgres) is a day's work because routes/db/engine are separated from the start |
| D39 | **Apple Developer Program: enroll as an individual (2026-09-25, founder).** Nurgazy enrolls personally ($99 USD/yr) to unblock Sign in with Apple, App Groups on device and TestFlight (M9). **Individual vs the founder's consulting corp is re-decided before public App Store launch, alongside P2.** | Apple docs checked 2026-09-25: an individual can sell paid apps, subscriptions and in-app purchases (Paid Apps Agreement plus tax and banking details) and qualifies for the Small Business Program's 15% commission. The trade-off is that the seller shown is the founder's legal name, and EU sales publicly display an address (a P.O. box is allowed), phone number and email under the DSA. Organization enrollment needs a D-U-N-S number (free, up to 7 business days), an email on the company's own domain and a public website. The corp has none of these, so it means a week of setup for benefits (seller name, liability, no personal address) that only matter at public launch. Apple support can convert an individual account to an organization; its docs don't say what happens to live apps, which is why the switch comes before launch |
...
- Seller entity **open** (D39): individual developer account now; move to the consulting
  corp (needs D-U-N-S + domain email + website) before public App Store launch, decided
  together with P2.
