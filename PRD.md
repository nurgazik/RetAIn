...
| D14 | **Taste guardrail:** no vocabulary embedding in tragedy/grim hard news; selection also requires **rewrite affordance** | Highlighted words in a disaster story would be ghoulish and poison the product's tone; texture-less pieces give advanced words nowhere to land (see §4) |
...
| D19 | **Re-amended 2026-07-31 (founder): QC gate DESCOPED for PoC** — no post-generation check while the rewrite model is Flash-Lite. Gate design is retained (per-word native-writer check, ~$0.001, demote-don't-delete UX) and **must be revisited when any trigger fires**: (a) before MVP build starts, (b) the Haiku fallback fires more than rarely (fallback path is ungated and has a measured 1–3 misuses/piece), (c) any close-read finds a hard misuse in Flash-Lite output, (d) model change | History: original posture was no-QC; 2026-07-26 evidence (Haiku-era: 1–3 misuses/piece, learner can't self-QC, prompt tightening insufficient) flipped it to gate-on; 2026-07-31 descoped because the evidence was model-specific — Flash-Lite runs 14/16 clean, zero hard breaks, and natively skips ill-fitting words. Accepted residual risk: ~90% ≠ 100% at PoC scale |
