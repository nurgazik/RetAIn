...
final class SafariActionTests: XCTestCase {
    ...
    func testShareFromSafari() throws {
        ...
        for _ in 0..<3 { let close = safari.buttons["Close"].firstMatch; if close.exists { close.tap(); sleep(1) } }
        ...
        for attempt in 0..<6 {
            ...
            let alt = [safari.buttons["RetAIn"], safari.cells["RetAIn"], safari.staticTexts["RetAIn"]]
        ...
        // The product sheet shows a WKWebView; its body text is exposed as staticTexts. The
        // reader kicker "YOUR READ" appears only once the piece has rendered.
        let ok = safari.staticTexts.matching(NSPredicate(format: "label CONTAINS[c] 'your read'")).firstMatch
        let bad = safari.staticTexts.matching(NSPredicate(format: "label CONTAINS[c] 'transform this' OR label CONTAINS[c] 'sign in' OR label CONTAINS[c] 'nothing to read'")).firstMatch
        let t0 = Date()
