...
| (b) Native-app share payloads | **Device-confirmed 2026-09-25 (founder's iPhone 16 Pro, iOS 26, spike build):** two text selections shared from a native app arrived as `public.plain-text` (639 and 742 chars); no URL, no page payload. Both transformed (2 words each). X, Kindle, Apple News still unlogged. | Spike log on the phone: `types: public.plain-text`, `text: 639 chars` / `742 chars` |
